...
class weights():
    def __init__(self,est,lmax,clfile,totalcls=None,unlclfile=None,u=None):
        ...
        if est=='TE_GMV_PRF':
            ...
            self.w[1][0]=f1*clte[:lmax+1]; self.w[1][1]=-1*f1*clee[:lmax+1]; self.w[1][2]=f2; self.s[1][0]=0; self.s[1][1]=0; self.s[1][2]=0  
        ...
        if est=='TB_GMV_PRF':
            ...
            f1 = u
            f2 = 1/u
            self.w[0][0]=0; self.w[0][1]=0; self.w[0][2]=0; self.s[0][0]=0; self.s[0][1]=0; self.s[0][2]=0  
        ...
        if est=='TE_GMV':
            #TODO: technically rows 24 and on should have cols 0 and 1 flipped, but as long as it's part of the TE weights, it shouldn't?
            self.sltt = sltt
            self.slee = slee
            self.slte = slte
            #self.ntrm = 24
