...
def analyze(sims=np.arange(99)+1,n0_n1_sims=np.arange(98)+1,
    ...
    for i, est in enumerate(ests):
        resps[:,i] = get_sim_response(est,config,cinv=True,append=append,sims=sims)
    ...
    for ii, sim in enumerate(sims):
        ...
        if n0 and n1:
            ...
            auto_debiased_TT = auto_TT - n0_cinv_TT - n1_cinv_TT
        elif n0:
    ...
    auto_cinv_avg_TT = auto_cinv_all_TT / num
...
def get_sim_response(est,config,cinv,append,sims,filename=None):
    '''
...
#analyze()
sims=np.arange(249)+1
n0_n1_sims=np.arange(248)+1
append='standard'
#config_file_list=['test_yuka_lmaxT3500.yaml','test_yuka_lmaxT4000.yaml']
config_file_list=['test_yuka.yaml']
