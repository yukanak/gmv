...
def main():
    ...
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_T1T1.npy'
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if append == 'mh' or append == 'mh_unl':
        ...
        alm2all[:,0] = hp.almxfl(tlm2_mv,invDl2)
        # T2T1
        alm1all[:,1] = hp.almxfl(tlm1_mv,invDl2)
