...
def main():
    ...
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_12ests.npy'
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if append == 'mh' or append == 'mh_unl':
        ...
        # TE
        alm1all[:,4] = hp.almxfl(tlm1_mv,invDl3)
        ...
        alm2all[:,7] = hp.almxfl(tlm2_mv,invDl3)
        # TB
        alm1all[:,8] = hp.almxfl(tlm1_mv,invDl3)
        alm2all[:,8] = hp.almxfl(blm2,flb)
        # BT
        alm1all[:,9] = hp.almxfl(blm1,flb)
        alm2all[:,9] = hp.almxfl(tlm2_mv,invDl3)
        # EB
        ...
        alm2all[:,3] = hp.almxfl(elm2,invDl3)
        # TE
        alm1all[:,4] = hp.almxfl(tlm1,invDl3)
        alm2all[:,4] = hp.almxfl(elm2,invDl3)
        # T2E1
        ...
        # E2T1
        alm1all[:,7] = hp.almxfl(elm1,invDl3)
        alm2all[:,7] = hp.almxfl(tlm2,invDl3)
        # TB
        alm1all[:,8] = hp.almxfl(tlm1,invDl3)
        alm2all[:,8] = hp.almxfl(blm2,flb)
        # BT
        alm1all[:,9] = hp.almxfl(blm1,flb)
        alm2all[:,9] = hp.almxfl(tlm2,invDl3)
        # EB
        alm1all[:,10] = hp.almxfl(elm1,invDl3)
        alm2all[:,10] = hp.almxfl(blm2,flb)
        # BE
