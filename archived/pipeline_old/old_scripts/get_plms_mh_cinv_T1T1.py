...
def main():
    ...
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv_T1T1.npy'
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if os.path.isfile(totalcls_filename):
        ...
        cltt1 = totalcls[:,0]; cltt2 = totalcls[:,0]; clttx = totalcls[:,0]; cltt3 = totalcls[:,0]; clee = totalcls[:,1]; clbb = totalcls[:,2]; clte = totalcls[:,3]
    ...
    if append == 'mh' or append == 'mh_unl':
        ...
        elif qe[:2] == 'T2': almbar1 = hp.almxfl((hp.almxfl(tlm1_mv,clee)-hp.almxfl(elm1,clte)),invDl2)
        ...
        elif qe[2:4] == 'T2': almbar2 = hp.almxfl((hp.almxfl(tlm2_mv,clee)-hp.almxfl(elm2,clte)),invDl2)
        elif qe[1] == 'T': almbar2 = hp.almxfl((hp.almxfl(tlm2_mv,clee)-hp.almxfl(elm2,clte)),invDl2)
        elif qe[1] == 'E': almbar2 = hp.almxfl((hp.almxfl(elm2,cltt2)-hp.almxfl(tlm2_mv,clte)),invDl2)
