...
import qest_separate_gmv as qest
...
def main():
    ...
    #filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3_12ests_nulledclte.npy'
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3_12ests_cutweights.npy'

    if False:#os.path.isfile(filename_gmv):
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if append == 'mh' or append == 'mh_unl':
        ...
        alm1all = np.zeros((len(tlm1_mv),12), dtype=np.complex_)
        alm2all = np.zeros((len(tlm2_tszn),12), dtype=np.complex_)
        # T1T2
        ...
        # T2E1
        alm1all[:,5] = hp.almxfl(tlm1_tszn,invDl2)
        alm2all[:,5] = hp.almxfl(elm2,invDl1)
        # ET
        alm1all[:,6] = hp.almxfl(elm1,invDl1)
        alm2all[:,6] = hp.almxfl(tlm2_tszn,invDl2)
        # E2T1
        alm1all[:,7] = hp.almxfl(elm1,invDl2)
        alm2all[:,7] = hp.almxfl(tlm2_mv,invDl1)
        # TB
        alm1all[:,8] = hp.almxfl(tlm1_mv,invDl1)
        alm2all[:,8] = hp.almxfl(blm2,flb)
        # BT
        alm1all[:,9] = hp.almxfl(blm1,flb)
        alm2all[:,9] = hp.almxfl(tlm2_tszn,invDl2)
        # EB
        ...
        alm1all = np.zeros((len(tlm1),12), dtype=np.complex_)
        alm2all = np.zeros((len(tlm2),12), dtype=np.complex_)
        # T1T2
        ...
        alm2all[:,4] = hp.almxfl(elm2,invDl2)
        # T2E1
        alm1all[:,5] = hp.almxfl(tlm1,invDl2)
        alm2all[:,5] = hp.almxfl(elm2,invDl1)
        # ET
        alm1all[:,6] = hp.almxfl(elm1,invDl1)
        ...
        # E2T1
        alm1all[:,7] = hp.almxfl(elm1,invDl2)
        alm2all[:,7] = hp.almxfl(tlm2,invDl1)
        # TB
    ...
    # Set clte to 0
    #totalcls[:,3] = 0;# totalcls[:,9] = 0; totalcls[:,10] = 0
    glm,clm,retdict = q_gmv.eval(qe,alm1all,alm2all,totalcls,crossilc=True)
    # Save plm and clm
    #Path(dir_out).mkdir(parents=True, exist_ok=True)
    #np.save(filename_gmv,glm)
    # This one only has the first term for TE, ET; just a test estimator run
    filename_gmv_dict = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3_12ests_cutweights_dict.npy'
    np.save(filename_gmv_dict,retdict)
