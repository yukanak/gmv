...
def main():
    ...
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv_T2T2.npy'
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if os.path.isfile(totalcls_filename):
        ...
        cltt1 = totalcls[:,5]; cltt2 = totalcls[:,5]; clttx = totalcls[:,5]; cltt3 = totalcls[:,5]; clee = totalcls[:,1]; clbb = totalcls[:,2]; clte = totalcls[:,10]
    ...
    if append == 'mh' or append == 'mh_unl':
        if qe[:2] == 'T1': almbar1 = hp.almxfl((hp.almxfl(tlm1_tszn,clee)-hp.almxfl(elm1,clte)),invDl1)
        elif qe[:2] == 'T2': almbar1 = hp.almxfl((hp.almxfl(tlm1_tszn,clee)-hp.almxfl(elm1,clte)),invDl2)
        elif qe[0] == 'T': almbar1 = hp.almxfl((hp.almxfl(tlm1_tszn,clee)-hp.almxfl(elm1,clte)),invDl1)
        elif qe[0] == 'E': almbar1 = hp.almxfl((hp.almxfl(elm1,cltt1)-hp.almxfl(tlm1_tszn,clte)),invDl1)
        elif qe[0] == 'B': almbar1 = hp.almxfl(blm1,flb)

        if qe[2:4] == 'T1': almbar2 = hp.almxfl((hp.almxfl(tlm2_tszn,clee)-hp.almxfl(elm2,clte)),invDl1)
