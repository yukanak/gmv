...
def analyze(sims=np.arange(250)+1,n0_n1_sims=np.arange(249)+1,
    ...
    for i, est in enumerate(ests):
        resps[:,i] = get_sim_response(est,config,cinv=True,append=append,sims=sims,gmv=True)
    ...
    if compare and sqe:
        ...
        for i, est in enumerate(ests):
            resps_sqe[:,i] = get_sim_response(est,config,cinv=False,gmv=False,append=append,sims=sims)
        ...
        resp_gmv = get_sim_response('all',config,cinv=False,append=append,sims=sims)
    ...
    if n0:
        ...
        if compare and sqe:
            n0_sqe = get_n0(sims=n0_n1_sims,qetype='sqe',config=config,append=append)
            n0_sqe_total = n0_sqe['total'] * (l*(l+1))**2/4
        elif compare:
            n0_gmv = get_n0(sims=n0_n1_sims,qetype='gmv',config=config,append=append)
    ...
    if n1:
        ...
        if compare and sqe:
            n1_sqe = get_n1(sims=n0_n1_sims,qetype='sqe',config=config,append=append)
            n1_sqe_total = n1_sqe['total'] * (l*(l+1))**2/4
        elif compare:
            n1_gmv = get_n1(sims=n0_n1_sims,qetype='gmv',config=config,append=append)
    ...
    for ii, sim in enumerate(sims):
        ...
        if os.path.isfile(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy'):
            plm = np.load(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')
        else:
            plms = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')),len(ests)), dtype=np.complex_)
            for i, est in enumerate(ests):
            ...
            np.save(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy',plm)
            for i, est in enumerate(ests):
                os.remove(dir_out+f'/plm_{est}_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')
        ...
        if compare and sqe:
            # Load SQE plms
            if os.path.isfile(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy'):
                plm_sqe = np.load(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
            else:
                plms_sqe = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_sqe[:,i] = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
                plm_sqe = np.sum(plms_sqe, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy',plm_sqe)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
    ...
    if not os.path.isfile(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_cinv.npy'):
        np.save(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_cinv.npy',uncertainty_cinv)
        np.save(dir_out+f'/agora_reconstruction/binned_measurement_uncertainty_lmaxT{lmaxT}_{append}_cinv.npy',binned_uncertainty_cinv)
    if compare and sqe:
        uncertainty_sqe = np.std(temp_sqe,axis=0)
        binned_uncertainty_sqe = np.std(binned_temp_sqe,axis=0)
        if not os.path.isfile(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_sqe.npy'):
            np.save(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_sqe.npy',uncertainty_sqe)
            np.save(dir_out+f'/agora_reconstruction/binned_measurement_uncertainty_lmaxT{lmaxT}_{append}_sqe.npy',binned_uncertainty_sqe)
    elif compare:
        uncertainty_gmv = np.std(temp_gmv,axis=0)
        binned_uncertainty_gmv = np.std(binned_temp_gmv,axis=0)
        if not os.path.isfile(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_gmv.npy'):
            np.save(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_gmv.npy',uncertainty_gmv)
            np.save(dir_out+f'/agora_reconstruction/binned_measurement_uncertainty_lmaxT{lmaxT}_{append}_gmv.npy',binned_uncertainty_gmv)
...
def get_n0(sims,qetype,config,append,cmbonly=False):
    ...
    if os.path.isfile(filename):
        ...
        # GMV response
        resp_gmv = get_sim_response('all',config,cinv=False,append=append_original,sims=np.append(sims,num+1))
        ...
        inv_resps = np.zeros((len(l),len(ests)) ,dtype=np.complex_)
        for i, est in enumerate(ests):
            resps[:,i] = get_sim_response(est,config,cinv=True,append=append_original,sims=np.append(sims,num+1))
            inv_resps[1:,i] = 1/(resps)[1:,i]
        resp = np.sum(resps, axis=1)
        ...
        for i, sim1 in enumerate(sims):
            ...
            # Get the lensed ij sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy'):
                plm_total_ij = np.load(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')
            else:
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                np.save(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy',plm_total_ij)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')

            # Now get the ji sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy'):
                plm_total_ji = np.load(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')
            else:
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                np.save(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy',plm_total_ji)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_gmv_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cinv.npy')
        ...
        inv_resps = np.zeros((len(l),len(ests)) ,dtype=np.complex_)
        for i, est in enumerate(ests):
            resps[:,i] = get_sim_response(est,config,gmv=False,cinv=False,append=append_original,sims=np.append(sims,num+1))
            inv_resps[1:,i] = 1/(resps)[1:,i]
        resp = np.sum(resps, axis=1)
        resp_TTEETE = resps[:,0]+resps[:,1]+resps[:,2]+resps[:,3]
        resp_TBEB = resps[:,4]+resps[:,5]+resps[:,6]+resps[:,7]
        ...
        for i, sim1 in enumerate(sims):
            ...
            # Get the lensed ij sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy'):
                plm_total_ij = np.load(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
            else:
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ij[:,i] = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
                plm_total_ij = np.sum(plms_ij, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy',plm_total_ij)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')

            # Now get the ji sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy'):
                plm_total_ji = np.load(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
            else:
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ji[:,i] = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
                plm_total_ji = np.sum(plms_ji, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy',plm_total_ji)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
...
def get_n1(sims,qetype,config,append):
    ...
    if os.path.isfile(filename):
        ...
        # GMV response
        resp_gmv = get_sim_response('all',config,cinv=False,append=append,sims=np.append(sims,num+1))
        ...
        inv_resps = np.zeros((len(l),len(ests)),dtype=np.complex_)
        for i, est in enumerate(ests):
            resps[:,i] = get_sim_response(est,config,cinv=True,append=append,sims=np.append(sims,num+1))
            inv_resps[1:,i] = 1/(resps)[1:,i]
        resp = np.sum(resps, axis=1)
        inv_resp = np.zeros_like(l,dtype=np.complex_); inv_resp[1:] = 1/(resp)[1:]
        ...
        for i, sim in enumerate(sims):
            # Get the lensed ij sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy'):
                plm_total_ij = np.load(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy')
            else:
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                plm_total_ij = np.sum(plms_ij, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy',plm_total_ij)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy')

            # Now get the ji sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy'):
                plm_total_ji = np.load(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy')
            else:
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                plm_total_ji = np.sum(plms_ji, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy',plm_total_ji)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy')
        ...
        for i, sim in enumerate(sims):
            # Get the lensed ij sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy'):
                plm_total_ij = np.load(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')
            else:
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ij[:,i] = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')
                plm_total_ij = np.sum(plms_ij, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy',plm_total_ij)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')

            # Now get the ji sims
            if os.path.isfile(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy'):
                plm_total_ji = np.load(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')
            else:
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ji[:,i] = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')
                plm_total_ji = np.sum(plms_ji, axis=1)
                np.save(dir_out+f'/plm_summed_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy',plm_total_ji)
                for i, est in enumerate(ests):
                    os.remove(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')
...
def get_sim_response(est,config,cinv,append,sims,filename=None,gmv=True):
    '''
    ...
    if os.path.isfile(filename):
        ...
        for ii, sim in enumerate(sims):
            ...
            if cinv:
                ...
                plm = np.load(dir_out+f'/plm_{est}_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
            if np.any(np.isnan(plm)):
                print(f'Sim {sim} is bad!')
                num -= 1
                continue
...
analyze(config_file='test_yuka.yaml',compare=True,sqe=True)
analyze(config_file='test_yuka_lmaxT3500.yaml',compare=True,sqe=True)
analyze(config_file='test_yuka_lmaxT4000.yaml',compare=False)
