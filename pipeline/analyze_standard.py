...
def analyze(sims=np.arange(250)+1,n0_n1_sims=np.arange(249)+1,
    ...
    if n0:
        ...
        if compare and sqe:
            ...
            n0_sqe_EE = n0_sqe['EE'] * (l*(l+1))**2/4
            n0_sqe_TE = n0_sqe['TE'] * (l*(l+1))**2/4
            n0_sqe_ET = n0_sqe['ET'] * (l*(l+1))**2/4
            n0_sqe_TB = n0_sqe['TB'] * (l*(l+1))**2/4
            n0_sqe_BT = n0_sqe['BT'] * (l*(l+1))**2/4
            n0_sqe_EB = n0_sqe['EB'] * (l*(l+1))**2/4
            n0_sqe_BE = n0_sqe['BE'] * (l*(l+1))**2/4
        elif compare:
    ...
    if n1:
        ...
        n1_cinv_TT = n1_cinv['TT'] * (l*(l+1))**2/4
        ...
        if compare and sqe:
            ...
            n1_sqe_TT = n1_sqe['TT'] * (l*(l+1))**2/4
            n1_sqe_EE = n1_sqe['EE'] * (l*(l+1))**2/4
            n1_sqe_TE = n1_sqe['TE'] * (l*(l+1))**2/4
            n1_sqe_ET = n1_sqe['ET'] * (l*(l+1))**2/4
            n1_sqe_TB = n1_sqe['TB'] * (l*(l+1))**2/4
            n1_sqe_BT = n1_sqe['BT'] * (l*(l+1))**2/4
            n1_sqe_EB = n1_sqe['EB'] * (l*(l+1))**2/4
            n1_sqe_BT = n1_sqe['BE'] * (l*(l+1))**2/4
        elif compare:
    ...
    for ii, sim in enumerate(sims):
        ...
        plm_resp_corr_TT = hp.almxfl(plms[:,0],inv_resps[:,0])
        plm_resp_corr_EE = hp.almxfl(plms[:,1],inv_resps[:,1])
        plm_resp_corr_TE = hp.almxfl(plms[:,2],inv_resps[:,2])
        plm_resp_corr_ET = hp.almxfl(plms[:,3],inv_resps[:,3])
        plm_resp_corr_TB = hp.almxfl(plms[:,4],inv_resps[:,4])
        plm_resp_corr_BT = hp.almxfl(plms[:,5],inv_resps[:,5])
        plm_resp_corr_EB = hp.almxfl(plms[:,6],inv_resps[:,6])
        plm_resp_corr_BE = hp.almxfl(plms[:,7],inv_resps[:,7])

        # Get spectra
        auto = hp.alm2cl(plm_resp_corr, plm_resp_corr, lmax=lmax) * (l*(l+1))**2/4
        auto_TT = hp.alm2cl(plm_resp_corr_TT, plm_resp_corr_TT, lmax=lmax) * (l*(l+1))**2/4
        ...
        if compare and sqe:
            # Load SQE plms
            plms_sqe = np.zeros((len(np.load(dir_out+f'/plm_TT_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')),len(ests)), dtype=np.complex_)
            for i, est in enumerate(ests):
                plms_sqe[:,i] = np.load(dir_out+f'/plm_{est}_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}.npy')
    ...
    if compare and sqe:
        ...
        #np.save(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_sqe.npy',uncertainty)
        #np.save(dir_out+f'/agora_reconstruction/binned_measurement_uncertainty_lmaxT{lmaxT}_{append}_sqe.npy',binned_uncertainty)
    elif compare:
        uncertainty = np.zeros((len(l),2), dtype=np.complex_)
        binned_uncertainty = np.zeros((len(bin_centers),2), dtype=np.complex_)
        uncertainty[:,0] = np.std(temp_gmv,axis=0)
        binned_uncertainty[:,0] = np.std(binned_temp_gmv,axis=0)
        #np.save(dir_out+f'/agora_reconstruction/measurement_uncertainty_lmaxT{lmaxT}_{append}_gmv.npy',uncertainty)
        #np.save(dir_out+f'/agora_reconstruction/binned_measurement_uncertainty_lmaxT{lmaxT}_{append}_gmv.npy',binned_uncertainty)
    ...
    plt.xlabel('$\ell$')
    plt.title(f'Spectra Averaged over {num} Sims, Standard')
...
def get_n0(sims,qetype,config,append,cmbonly=False):
    ...
    if os.path.isfile(filename):
        ...
        # GMV response
        resp_gmv = get_sim_response('all',config,cinv=False,append=append_original,sims=np.append(sims,
num+1))
...
def get_n1(sims,qetype,config,append):
    ...
    if os.path.isfile(filename):
        ...
        for i, sim in enumerate(sims):
            # Get the lensed ij sims
            plm_TT_ij = np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_cinv.npy')
            ...
            # Now get the ji sims
            plm_TT_ji = np.load(dir_out+f'/plm_TT_healqest_gmv_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_cinv.npy')
...
analyze(config_file='test_yuka.yaml')
#analyze(config_file='test_yuka_lmaxT3500.yaml')
#analyze(config_file='test_yuka_lmaxT4000.yaml')
