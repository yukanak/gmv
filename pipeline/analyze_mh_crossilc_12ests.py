...
def analyze(sims=np.arange(250)+1,n0_n1_sims=np.arange(249)+1,
    ...
    for ii, sim in enumerate(sims):
        ...
        if compare and sqe:
            # Load SQE plms
            plms_sqe = np.zeros((len(np.load(dir_out+f'/plm_EE_healqest_sqe_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy')),len(ests)), dtype=np.complex_)
            for i, est in enumerate(ests):
                if withT3:
...
analyze(append='mh',config_file='test_yuka_lmaxT3500.yaml',compare=True, withT3=False)
#analyze(append='mh',config_file='test_yuka_lmaxT4000.yaml')
#analyze(append='crossilc_twoseds',config_file='test_yuka_lmaxT3500.yaml',compare=True, withT3=False)
