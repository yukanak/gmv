...
#append = f'tsrc_fluxlim{fluxlim:.3f}_oldGMVTEweights'
...
if not gmv:
    ...
    np.save(dir_out+f'/plm_{qe}_healqest_seed1_{sim1}_seed2_{sim2}_lmax{lmax}_nside{nside}_{append}.npy',glm)
    #np.save(dir_out+f'/clm_{qe}_healqest_seed1_{sim1}_seed2_{sim2}_lmax{lmax}_nside{nside}_{append}.npy',clm)
else:
    ...
    if qe == 'TBEB':
        ...
        np.save(dir_out+f'/plm_TTEETEprf_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmax{lmax}_nside{nside}_{append}.npy',glm_prf)
    glm,clm = q_gmv.eval()
    # Save plm and clm
    Path(dir_out).mkdir(parents=True, exist_ok=True)
    np.save(dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmax{lmax}_nside{nside}_{append}.npy',glm)
    #np.save(dir_out+f'/clm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmax{lmax}_nside{nside}_{append}.npy',clm)
