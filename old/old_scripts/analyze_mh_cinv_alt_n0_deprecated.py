...
def analyze(sims=np.arange(99)+1,n0_n1_sims=np.arange(98)+1,
    ...
    if n0:
        ...
        n0_cinv_T1T2 = n0_cinv['T1T2'] * (l*(l+1))**2/4
        n0_cinv_T2T1 = n0_cinv['T2T1'] * (l*(l+1))**2/4
    ...
    if n1:
        ...
        n1_cinv_T2T1 = n1_cinv['T2T1'] * (l*(l+1))**2/4
    ...
    for ii, sim in enumerate(sims):
        ...
        if n0 and n1:
            ...
            auto_debiased_TT = (auto_T1T2+auto_T2T1)*0.5 - 0.5*(n0_cinv_T1T2+n0_cinv_T2T1) - 0.5*(n1_cinv_T1T2+n1_cinv_T2T1)
        elif n0:
    ...
    #plt.ylim(0.98,1.02)
    plt.xlim(10,lmax)
    plt.savefig(dir_out+f'/figs/{num}_sims_comparison_{append}_cinv_resp_from_sims_n0n1subtracted_binnedratio_lmaxT{lmaxT}_alt_n0.png',bbox_inches='tight')
    ...
    #plt.ylim(0.98,1.02)
    plt.xlim(10,lmax)
    plt.savefig(dir_out+f'/figs/n0_comparison_ratio_gmv_{num}_sims_mh_cinv_resp_from_sims_alt_n0.png',bbox_inches='tight')
...
def get_n0(sims,qetype,config,append,cmbonly=False,withT3=True,T1T1=False,T2T2=False):
    ...
    if T1T1:
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n0/n0_{num}simpairs_healqest_{qetype}_T1T1_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_9ests.pkl'
    elif T2T2:
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n0/n0_{num}simpairs_healqest_{qetype}_T2T2_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_9ests.pkl'
    elif not withT3:
        if qetype == 'gmv':
        ...
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n0/n0_{num}simpairs_healqest_{qetype}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_9ests.pkl'
    ...
    if os.path.isfile(filename):
        ...
        for i, est in enumerate(ests):
            ...
            inv_resps[1:,i] = 1/(resps)[1:,i]
        resp = 0.5*resps[:,0]+0.5*resps[:,1]+np.sum(resps[:,2:], axis=1)
        resp_TTEETE = 0.5*resps[:,0]+0.5*resps[:,1]+resps[:,2]+resps[:,3]+resps[:,4]
        resp_TBEB = resps[:,5]+resps[:,6]+resps[:,7]+resps[:,8]
        inv_resp = np.zeros_like(l,dtype=np.complex_); inv_resp[1:] = 1/(resp)[1:]
...
def get_n1(sims,qetype,config,append,withT3=True,T1T1=False,T2T2=False):
    ...
    if T1T1:
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n1/n1_{num}simpairs_healqest_{qetype}_T1T1_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_n0_with_9ests.pkl'
    elif T2T2:
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n1/n1_{num}simpairs_healqest_{qetype}_T2T2_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_n0_with_9ests.pkl'
    elif not withT3:
        if qetype == 'gmv':
        ...
        if qetype == 'gmv':
            ...
            filename = dir_out+f'/n1/n1_{num}simpairs_healqest_{qetype}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_resp_from_sims_n0_with_9ests.pkl'
