...
def analyze(sims=np.arange(99)+1,n0_n1_sims=np.arange(98)+1,
    ...
    plt.xlabel('$\ell$')
    plt.title(f'Spectra Averaged over {num} Sims, Cross-ILC')
...
num+1))
        ...
        # SQE response
        # NOT including T2T1 in N0 calculation
...
for append in append_list:
    for config_file in config_file_list:
        ...
        for i, est in enumerate(ests):
            resps_original[:,i] = get_sim_response(est,config,gmv=False,append=append,sims=sims)
        # Get N0
        n0_original = get_n0(sims=n0_n1_sims,qetype='sqe',config=config,append=append)
        # Get N1
        n1_original = get_n1(sims=n0_n1_sims,qetype='sqe',config=config,append=append)
