...
class gmv_resp(object):
    ...
    def f_XY(self, L, l_1, phi1, XY, l2l1=False):
        """
        ...
        if l2l1:
            if XY == 'TT':
                ...
                result = self.slte(l_2)*np.cos(2.*phi21)*Ldotl_2
                result += self.slte(l_1)*Ldotl_1
            elif XY == 'TB':
                result = self.slte(l_2)*np.sin(2.*phi21)*Ldotl_2
            elif XY == 'EB':
                # There is a typo in HO02!!!!!!!!!
    ...
    def f(self, L, l_1, phi1, l2l1=False):

        f = np.zeros((len(l_1), 2, 2))

        #TODO
        #f[:,0,0] = self.f_XY(L, l_1, phi1, 'TT', l2l1=l2l1)
        f[:,0,0] = self.f_XY(L, l_1, phi1, 'EE', l2l1=l2l1)
        #f[:,1,1] = self.f_XY(L, l_1, phi1, 'EE', l2l1=l2l1)
        f[:,1,1] = self.f_XY(L, l_1, phi1, 'BB', l2l1=l2l1)
        #f[:,2,2] = self.f_XY(L, l_1, phi1, 'BB', l2l1=l2l1)
        #f[:,0,1] = f[:,1,0] = self.f_XY(L, l_1, phi1, 'TE', l2l1=l2l1)
        f[:,0,1] = f[:,1,0] = 0
        #f[:,0,2] = f[:,2,0] = self.f_XY(L, l_1, phi1, 'TB', l2l1=l2l1)
        #f[:,1,2] = f[:,2,1] = self.f_XY(L, l_1, phi1, 'EB', l2l1=l2l1)
    ...
    def A(self, L):
        ...
        def integrand(l_1, phil, semi=self.semi):
            ...
            if semi:
                ...
                fl2l1 = self.f(L, l_1, phil, l2l1=True)
