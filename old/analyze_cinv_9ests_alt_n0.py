...
def analyze(sims=np.arange(99)+1,n0_n1_sims=np.arange(98)+1,
    ...
    for i, est in enumerate(ests):
        resps_original[:,i] = get_sim_response(est,config,gmv=False,cinv=False,append=append,sims=sims,withT3=withT3)
    ...
    if n0:
        ...
        n0_original_TT = 0.5*(n0_original['T1T2']+n0_original['T2T1']) * (l*(l+1))**2/4
    if n1:
        ...
        n1_original_TT = 0.5*(n1_original['T1T2']+n1_original['T2T1']) * (l*(l+1))**2/4
    ...
    temp_cinv_TTEETE = np.zeros((len(sims),len(l)),dtype=np.complex_)
    ...
    binned_temp_cinv_TTEETE = np.zeros((len(sims),len(l)),dtype=np.complex_)
    ...
    for ii, sim in enumerate(sims):
        ...
        if not withT3:
            ...
            # SQE
            plms_original = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy')),len(ests)), dtype=np.complex_)
            for i, est in enumerate(ests):
                plms_original[:,i] = np.load(dir_out+f'/plm_{est}_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy')
            plm_original = 0.5*np.sum(plms_original[:,:2], axis=1)+np.sum(plms_original[:,2:], axis=1)
            plm_original_TTEETE = 0.5*np.sum(plms_original[:,:2], axis=1)+np.sum(plms_original[:,2:5], axis=1)
            plm_original_TBEB = np.sum(plms_original[:,5:], axis=1)
        else:
            # Load GMV plms
        ...
        if n0 and n1:
            ...
            auto_original_debiased_TT = (auto_original_T1T2+auto_original_T2T1)*0.5 - n0_original_TT - n1_original_TT
        elif n0:
            ...
            auto_original_debiased_TT = (auto_original_T1T2+auto_original_T2T1)*0.5 - n0_original_TT
        ...
        if n0:
            ...
            temp_cinv_TTEETE[ii,:] = auto_debiased_TTEETE
            ...
            binned_temp_cinv_TTEETE = [auto_debiased_TTEETE[digitized == i].mean() for i in range(1, len(lbins))]
    ...
    uncertainty[:,1] = np.std(temp_cinv_TTEETE,axis=0)
    binned_uncertainty[:,0] = np.std(binned_temp_cinv,axis=0)
    binned_uncertainty[:,1] = np.std(binned_temp_cinv_TTEETE,axis=0)
...
def get_n0(sims,qetype,config,append,cmbonly=False,withT3=True):
    ...
    if os.path.isfile(filename):
        ...
        for i, sim1 in enumerate(sims):
            ...
            if not withT3:
                # Get the lensed ij sims
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                # Now get the ji sims
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim2}_seed2_{sim1}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
...
def get_n1(sims,qetype,config,append,withT3=True):
    ...
    if os.path.isfile(filename):
        ...
        for i, sim in enumerate(sims):
            if not withT3:
                # Get the lensed ij sims
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2_noT3.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                # Now get the ji sims
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1_noT3.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                ...
                # Get the lensed ij sims
                plms_ij = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ij[:,i] = np.load(dir_out+f'/plm_{est}_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu1tqu2.npy')
                plm_total_ij = 0.5*np.sum(plms_ij[:,:2], axis=1)+np.sum(plms_ij[:,2:], axis=1)

                # Now get the ji sims
                plms_ji = np.zeros((len(np.load(dir_out+f'/plm_T1T2_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')),len(ests)), dtype=np.complex_)
                for i, est in enumerate(ests):
                    plms_ji[:,i] = np.load(dir_out+f'/plm_{est}_healqest_seed1_{sim}_seed2_{sim}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_cmbonly_phi1_tqu2tqu1.npy')
                plm_total_ji = 0.5*np.sum(plms_ji[:,:2], axis=1)+np.sum(plms_ji[:,2:], axis=1)
...
withT3 = True
#append_list=['mh','crossilc_twoseds']
append_list=['crossilc_onesed']
