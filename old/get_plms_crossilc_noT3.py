...
def main():
    ...
    filename_sqe = dir_out+f'/plm_{qe}_healqest_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy'
    filename_gmv = dir_out+f'/plm_{qe}_healqest_gmv_seed1_{sim1}_seed2_{sim2}_lmaxT{lmaxT}_lmaxP{lmaxP}_nside{nside}_{append}_noT3.npy'
...
def do_reconstruction(qe,sim1,sim2,append,config_file):
    ...
    if not gmv:
        ...
        if append == 'crossilc_onesed' or append == 'crossilc_onesed_unl' or append == 'crossilc_twoseds' or append == 'crossilc_twoseds_unl':
            ...
            elif qe[0] == 'T': almbar1 = hp.almxfl(tlm1_cibn,flt1); flm1 = flt1
            ...
            elif qe[1] == 'T': almbar2 = hp.almxfl(tlm2_tszn,flt2); flm2 = flt2
            ...
            elif qe[1] == 'T': almbar2 = hp.almxfl(tlm2,flt2); flm2 = flt2
            elif qe[1] == 'E': almbar2 = hp.almxfl(elm2,fle); flm2 = fle
            elif qe[1] == 'B': almbar2 = hp.almxfl(blm2,flb); flm2 = flb
    else:
        ...
        if append == 'crossilc_onesed' or append == 'crossilc_onesed_unl' or append == 'crossilc_twoseds' or append == 'crossilc_twoseds_unl':
            ...
            alm2all[:,1] = hp.almxfl(tlm2_cibn,invDl1)
            # EE
            alm1all[:,2] = hp.almxfl(elm1,invDl1)
            alm2all[:,2] = hp.almxfl(elm2,invDl2)
            # TE
            alm1all[:,3] = hp.almxfl(tlm1_cibn,invDl1)
            alm2all[:,3] = hp.almxfl(elm2,invDl2)
            # ET
            alm1all[:,4] = hp.almxfl(elm1,invDl1)
            alm2all[:,4] = hp.almxfl(tlm2_tszn,invDl2)
            # TB
            alm1all[:,5] = hp.almxfl(tlm1_cibn,invDl1)
